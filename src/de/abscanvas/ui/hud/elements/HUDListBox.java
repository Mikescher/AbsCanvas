...
public class HUDListBox extends HUDElement {
	...
	public HUDListBox(int x, int y, int width, int height, HUD owner) {
	...
	public void tick() {
		...
		if (mouseDownOnScrollbar) {
			...
			if (mb.isMouseMoving()) {
				MDPoint p = owner.getOwner().getOwner().screenToCanvas(mb.getPosition());
