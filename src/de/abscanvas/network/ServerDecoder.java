...
public class ServerDecoder extends NetworkDecoder {
	private ServerAdapter owner;

	private byte packageID;

	public ServerDecoder(DatagramPacket p, byte[] data, NetworkListener l, ServerAdapter a) {
		super(p, data, l);
		owner = a;
	}
	...
	public void run() {
		if (data[0] == owner.getIdentifier()[0] && data[1] == owner.getIdentifier()[1] && data[2] == owner.getIdentifier()[2] && data[3] == owner.getIdentifier()[3]) {
			...
			listener.recievePackage(dater, packageID, packet.getAddress(), packet.getPort());
	...
	private void recieveCMD(byte[] d, InetAddress ip, int port, boolean first) {
		...
		if (command == NetworkConstants.CMD_BUFFER_END) {
			...
			NetworkUser u = listener.getServerUser(ip, port);
			if (u != null) {
				is_new = u.recieveReliablePackage(relUID);
			}
